...
const saltRounds = 10;
const jwtSecretyKey = '2AE458DA29E526148A71527CE19FB'; // Can be provided by extra file but for test reason I'm putting here
...
// Users route
route.post('/users/login', jsonParser, (req, res, next)=>{
...
// User data route
route.get('/users/me', jsonParser, checkToken, (req, res)=>{
...
// User logout route
route.get('/users/logout', jsonParser, checkToken, (req, res)=>{
...
route.post('/products', jsonParser, checkToken, (req, res)=>{
...
// fetch products
route.get('/products', jsonParser, checkToken, (req, res)=>{
    jwt.verify(req.token, jwtSecretyKey, (err, data) => {
        if(err){
            res.sendStatus(403);
        }else{
...
route.get('/products/page', jsonParser, checkToken, (req, res)=>{
    jwt.verify(req.token, jwtSecretyKey, (err, data) => {
        if(err){
            res.sendStatus(403);
        }else{
...
// fetch product by id
route.get('/products/:id', jsonParser, checkToken, (req, res)=>{
    jwt.verify(req.token, jwtSecretyKey, (err, data) => {
        if(err){
            res.sendStatus(403);
        }else{
...
// update product
route.put('/products/:id', jsonParser, checkToken, (req, res)=>{
    jwt.verify(req.token, jwtSecretyKey, (err, data) => {
        if(err){
            res.sendStatus(403);
        }else{
...
route.delete('/products/:id', jsonParser, checkToken, (req, res)=>{
    jwt.verify(req.token, jwtSecretyKey, (err, data) => {
        if(err){
            res.sendStatus(403);
        }else{
